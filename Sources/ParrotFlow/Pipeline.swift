...
struct Pipeline: Equatable, Codable {
    ...
    ///
    /// Like `context` this returns its input untouched by construction, and for
    /// the same reason: a stage that could put the field into the transcript
    /// would be a stage that could paste what you already typed back on top of
    /// itself.
    ///
    /// The box is not read here. It was read when the hotkey went down — see
    /// `InputBox.capturePress` — because by then focus may have moved and the
    /// question is what was in the box you were typing in, not what is in
    /// whatever is in front now.
    ///
    /// `caret`, `selected` and `appending` are absent rather than zero where the
    /// surface publishes no selection, which is every terminal. Absent throws in
    /// a condition and that is the point: `when: input.appending` should fail
    /// loudly on a surface that cannot answer it, not quietly read as "no".
    ...
    private func runTransform(
        ...
        case .replace:
            ...
            // cannot publish for itself, and what it wrote is exactly what a
            // later stage must not undo. `dotted` writes the dot in `user.name`
            // and `join` then reads it as one the decoder invented.
