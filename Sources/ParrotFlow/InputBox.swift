...
enum InputBox {
    ...
    struct Capture {
        ...
        /// rule read. Twenty characters a side covers a sentence end, a
        /// newline, a bracket and a hyphen, which is everything the rules look
        /// at.
        ///
        ...
        /// and reading it as text is how you see that in a glance.
    ...
    static func capturePress(app: Pipeline.App?, element: AXUIElement?) {
        ...
        case .success(let capture):
            ...
            // Press time and pipeline time are the same snapshot, so this
            // repeats the line the input stage writes. It is logged twice
            // because the two moments are seconds apart and anything typed
            // between them is invisible to both — a pipeline with no `input`
            // stage still leaves the evidence here.
