...
struct Config: Decodable, Equatable {
    ...
    /// not collide — and neither `--eval` nor `--pipeline` can pick a language
    /// today, so a per-language table would ship untestable.
    ...
    /// earlier in the same alternation. An unknown name is left alone rather
    /// than emptied: `(?:)` matches everywhere and would turn a guard into a
    /// rule that never fires, silently.
