...
while IFS='|' read -r name instruction input expect limit; do
  ...
  if [ "$MODEL" = 1 ]; then
    ...
    # Same reason the numbers cases pin `lang:`, twice over. The month a date
    # is spelled out in follows the languages this set assumes, and which
    # field leads in "3/12" follows the region it assumes — day-first here.
    # On a runner set to en_US the identical code answers "12/3" and is right
    # to; the set scored 23/27 there before it said which convention it was
    # asserting.
