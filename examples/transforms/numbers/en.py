...
Only the vocabulary and the three rules that differ are here: a tens word
takes at most one unit, a bare scale word is not a number, and "a" stands in
for one below a million. `engine.py` holds the rest, and how to add a language.

Score with `./score.py --lang en`, or `ParrotFlow --eval numbers_en`.
"""
import sys
...
GRAMMAR = engine.Grammar(
    ...
    hundred={"hundred"},
    # "second" is left out: it is a unit of time more often than an ordinal,
    # and "a thirty second timeout" must not become "a 32nd timeout".
    ordinal_units={
