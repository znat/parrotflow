...
A language is a `Grammar`: the word tables and the few rules that differ. This
file holds the rest — where a number begins and ends, years, decimals,
ordinals, percent, and the guard that keeps one grammar off another language's
sentence.

Adding a language is copying a file:

    cp fr.py es.py                    # edit GRAMMAR, nothing else here
...
that. Steps carry no language gate: every grammar reads every transcript.
    ...
    ParrotFlow --eval numbers_en                # the installed copy
    scripts/check-pipeline.sh                   # the order and the guard
"""
import json
import os
import re
...
# Below this a lone number word stays a word: "chapter three", and the pronoun
# "one". A compound converts whatever its size, so "one hundred" is 100.
DIGITS_FROM = 10
...
# A bare scale word stands in for one of itself only below this. "mille euros"
# is 1000 euros; from a million up the word is a noun and wants a number in
# front of it, so "des millions de personnes" is not 1000000 of them.
BARE_SCALE_LIMIT = 1000000
...
def parse_vigesimal(run, start, g):
    ...
    if (kind == "unit" and value == 4 and not item.ordinal
        ...
        # "dix-sept" is one number and arrives as two tokens. Only seven,
        # eight and nine: "dix un" is ten and one, not 11.
...
def parse_hundreds(run, start, g):
    ...
    # English wants "a hundred": "hundreds of people" is not a number.
...
def parse_number(run, start, g):
    ...
    while index < len(run):
        ...
        # of them.
        if (g.bare_scale_is_one and kind == "scale"
                and value < BARE_SCALE_LIMIT and not run[index].ordinal
