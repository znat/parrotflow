...
this stage's own name, and a later stage that re-cases or deletes words reads
that list and leaves them alone — the same contract `code_identifiers` uses for
the identifiers it builds. A quotation is verbatim by definition: `join`
capitalises the first word of a clip, and "quote we should ship this unquote"
must not come out as "We should ship this" when "we" is what was said.
